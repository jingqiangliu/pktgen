...
#define PKTGEN_MAGIC 0x00010100 //0xbe9be955
...
0x69, 0x76, 0x65, 0x64, 0x6e, 0x73, 0x03, 0x6f, 0x72, 0x67, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00,
				...
				int datalen)
{
	...
	if (pkt_dev->nfrags <= 0) {
		...
		unsigned char *data = skb_put(skb, datalen);
		for (i=0; i < datalen; i++)
		  *(data + i) = payload[i];
	...
	pgh->seq_num = htonl(0x00010000); //htonl(pkt_dev->seq_num);

//	do_gettimeofday(&timestamp);
	pgh->time = htonll(0x000109616374); //htonll(ktime_to_ns(ktime_get()));
					...
					struct pktgen_dev *pkt_dev)
{
	...
	*(__be16 *) & eth[12] = protocol;
	...
	udph->check = 0; //0x2e2a;	/* No checksum */
